...
//        add_filter( 'post_thumbnail_html', 'remove_width_attribute', 10 );
	...
	public function register_assets()
	{
		if (!is_admin()) {
			...
			wp_enqueue_script('mymenu-theme', get_template_directory_uri() . '/js/menu-theme.js', array('jquery'), false, true);

            wp_enqueue_style('mymenu-bootstrap', get_stylesheet_directory_uri() . '/css/bootstrap.min.css');
			/*
			 * Add CSS styles
			 */
			...
			wp_enqueue_style('mymenu-fonts', add_query_arg($args, (is_ssl() ? 'https' : 'http') . '://fonts.googleapis.com/css'),	array(), null);
			*/
			// add core
			if (is_rtl()) {
				wp_enqueue_style('mymenu-core', get_stylesheet_directory_uri() . '/css/rtl.css');
			}
			else {
				wp_enqueue_style('mymenu-core', get_stylesheet_uri());
			}
			
			/*if (!Bunyad::options()->no_responsive) {
				wp_enqueue_style('mymenu-responsive', get_template_directory_uri() . '/css/'. (is_rtl() ? 'rtl-' : '') . 'responsive.css');
			}*/

			// custom scheme
			if (Bunyad::options()->predefined_style) {
				wp_enqueue_style('mymenu-skin', get_template_directory_uri() . '/css/skin-' . Bunyad::options()->predefined_style . '.css');
			}
...
//			wp_enqueue_script('flex-slider', get_template_directory_uri() . '/js/jquery.flexslider-min.js', array('jquery'), false, true);
	...
	public function register_sidebars()
	{
        ...
        register_sidebar(array(
			'name' => __('Banner Sidebar', 'bunyad'),
			'id'   => 'banner-sidebar',
			'description' => __('Widgets in this area will be shown in the banner sidebar.', 'bunyad'),
            'before_title' => '',
            'after_title'  => '',
            'before_widget' => '',
            'after_widget'  => ''
		...
		// register dynamic sidebar
		register_sidebar(array(
			'name' => __('Footer', 'bunyad'),
			'id'   => 'main-footer',
			'description' => __('Widgets in this area will be shown in the footer.', 'bunyad'),
            'before_title' => '<h3 class="heading">',
            'after_title'  => '</h3>',
            'before_widget' => '<div class="col-md-4"><div class="footer-box">',
            'after_widget'  => '</div></div>'
		));

        register_sidebar(array(
            'name' => __('Footer Banner', 'bunyad'),
            'id'   => 'banner-footer',
            'description' => __('Widgets in this area will be shown in the footer.', 'bunyad'),
