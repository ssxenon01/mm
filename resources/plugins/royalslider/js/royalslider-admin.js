(function($) {
	...
	$(document).ready(function() {
		...
		function getTimThumbPath(imgPath, width, height) {
			var	thumbPath;			
			if(!wpmuEnabled) {
				thumbPath = imgPath;
			} else {				
			...
			return pluginUrl + "timthumb/timthumb.php?src=" + thumbPath  +'&w=' + width + '&h=' + height + '&q=100';
		}
		...
		function getSlideBgImage(slide) {
			...
			if(imgPath != undefined && imgPath != "") {			
				...
				if(getOptionFromMainSidebar('auto-generate-images')) {					
					imgPath = getTimThumbPath(imgPath,  ((wid.indexOf('%') < 0) ? wid : 600), ((hei.indexOf('%') < 0) ? hei : 400));
				}				
				return imgPath;
			}
			return false;
