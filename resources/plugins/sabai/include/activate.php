<?php
function sabai_wordpress_activate()
{
    ...
    try {
        $sabai = $sabai_wordpress->getSabai(true, true);
        // If no exception, the plugin is already installed so do nothing
        return;
    } catch (Sabai_NotInstalledException $e) {
        $sabai = $sabai_wordpress->getSabai(false); // get Sabai without loading addons
    }
    
    // Pre install
    $sabai_wordpress->updateSabaiOption('page_slugs', array(array(), array(), array()), true);
    $sabai_wordpress->updateSabaiOption('admin_menus', array(), true);
    
    // Install
    if (!sabai_wordpress_activate_addons($sabai, $addons, $log)) {
        
    }
    
    $log[] = 'done.';

    $sabai_wordpress->updateSabaiOption('install_log', implode('', (array)$log));
}

function sabai_wordpress_activate_addons(Sabai $sabai, array $addons, $log)
{    
    try {
