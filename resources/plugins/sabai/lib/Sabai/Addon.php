...
abstract class Sabai_Addon implements SabaiFramework_EventDispatcher_EventListener
{
    ...
    public function getInstallForm(array $parents = array())
    {
        // Override this to display default configuration settings form on install
        return array();
    }
        
    public function getUpgradeForm($currentVersion, array $parents = array())
    {
        // Override this to display default configuration settings form on upgrade
        return array();
    }
    ...
    final public function loadConfig($configName = null)
    {
        if (!$entity = $this->_application->getAddon('System')->getModel('Addon')->name_is($this->_name)->fetchOne()) {
            throw new RuntimeException(sprintf(__('Failed fetching the model entity for plugin %s', 'sabai'), $this->_name));
        }
    ...
    public function install(ArrayObject $log)
    {
        if ($latest_schema = $this->_hasSchema()) {
            ...
            $this->_application->getDBSchema()->create($latest_schema);
        }
        $this->_createVarDirs();
    ...
    public function uninstall(ArrayObject $log)
    {
        if ($latest_schema = $this->_hasSchema()) {
            if (false === $schema_old = $this->_getOlderSchemaList($this->getVersion())) {
                throw new RuntimeException(__('Failed fetching schema files.', 'sabai'));
            }
            ...
            $this->_application->getDBSchema()->drop($previous_schema);
        }
    ...
    public function upgrade($currentVersion, ArrayObject $log)
    {
        if ($this->_hasSchema()) {
            ...
            if (!empty($schema_new)) {
                ...
                $schema = $this->_application->getDBSchema();
                if (!empty($schema_old)) {
                    ...
                    $schema->create($previous_schema);
                    $this->_application->LogInfo(sprintf('Created database using schema %s.', basename($previous_schema)));
                }
                // update schema incrementally
                foreach ($schema_new as $new_schema) {
                    $schema->update($new_schema, $previous_schema);
                    $log[] = sprintf(
                        'Updated database schema from %s to %s.',
                        basename($previous_schema),
                        basename($new_schema)
                    );
    ...
    public function getPreviousAndNewSchema()
    {
        if (!$this->_hasSchema()) {
            return;
        }
        ...
        return array($previous_schema, $schema_new);
    }
    ...
    private function _getSchemaList()
    {
        ...
        if (false === $files = glob($schema_dir . '/*.xml', GLOB_NOSORT)) return false; // return false on error
        foreach ($files as $file) {
            if (preg_match('/^\d+(?:\.\d+)*(?:[a-zA-Z]+\d*)?\.xml$/', basename($file))) {
    ...
    {
        $schema_path = $this->_application->getAddonPath($this->_name) . '/schema/' . $version . '.xml';

        return file_exists($schema_path) ? $schema_path : false;
    }

    final public function getRequestedEntity(Sabai_Context $context, $entityName, $entityIdVar = null)
    {
        $entity_name_lc = strtolower($entityName);
        $entity_idvar = !isset($entityIdVar) ? $entity_name_lc . '_id' : $entityIdVar;
        if (0 < $entity_id = $context->getRequest()->asInt($entity_idvar)) {
            return $this->getModel($entityName)->fetchById($entity_id);
        }
    ...
    public function isUpgradeable($currentVersion, $newVersion)
    {
        if ($parent_addon = $this->hasParent()) {
            ...
            return $this->_application->CheckAddonVersion($parent_addon, $newVersion);
        }
        return true;
    }
    ...
    {
        return $this->_application->_($msgId, $this->getPackage());
    }

    public function _e($msgId)
    {
        echo $this->_application->_($msgId, $this->getPackage());
    }

    public function _n($msgId, $msgId2, $num)
    {
        return $this->_application->_n($msgId, $msgId2, $this->getPackage());
    }
    
    public function __($msgId, $context)
    {
        return $this->_application->__($msgId, $context, $this->getPackage());
    }

    public function __n($msgId, $msgId2, $num, $context)
    {
        return $this->_application->_n($msgId, $msgId2, $num, $context, $this->getPackage());
    }
}