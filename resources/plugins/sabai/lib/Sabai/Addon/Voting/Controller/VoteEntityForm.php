...
{    
        ...
        if ($vote) {
            ...
            if (isset($settings['form_other_option']) && isset($settings['form_options'][$settings['form_other_option']])) {
                $form['comment'] = array(
                    ...
                    '#states' => array(
                        'visible' => array(
                            'input[name="value[0]"]' => array('type' => 'value', 'value' => $settings['form_other_option']),
                        ),
                    ),
    ...
    target.hide();
    if (result.trashed) {
        jQuery("#%1$s").fadeTo("fast", 0, function(){jQuery(this).slideUp("medium", function(){jQuery(this).remove();});});
    } else {
