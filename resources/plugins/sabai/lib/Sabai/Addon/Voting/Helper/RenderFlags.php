...
';
        ...
        foreach ($flags as $flag) {
            $li[] = sprintf(
                $li_html,
                $flag->user_id ? $application->UserIdentityThumbnailSmall($flag->User) : '<i class="sabai-icon-large sabai-icon-exclamation-sign"></i>',
                $flag->user_id ? $application->UserIdentityLink($flag->User) : __('System Message', 'sabai'),
                $flag->id,
                $application->DateDiff($flag->created),
        ...
        $links = array(
            0 => $application->LinkToModal(__('Ignore Flags', 'sabai'), $application->Entity_Url($entity, '/voting/flags/ignore'), $link_options + array('icon' => 'ok'), array('class' => 'sabai-btn sabai-btn-success', 'title' => __('Ignore Flags', 'sabai'))),
        );
        $label = $application->Entity_BundleLabel($application->Entity_Bundle($entity), true);
        $links[2] = $application->LinkToModal(sprintf(__('Delete %s', 'sabai'), $label), $application->Entity_Url($entity, '/delete', array('delete_target_id' => 'sabai-entity-content-' . $entity->getId())), $link_options + array('icon' => 'trash'), array('class' => 'sabai-btn sabai-btn-danger', 'title' => sprintf(__('Delete %s', 'sabai'), $label)));
        ksort($links);
