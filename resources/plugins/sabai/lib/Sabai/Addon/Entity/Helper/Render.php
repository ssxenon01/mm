...
class Sabai_Addon_Entity_Helper_Render extends Sabai_Helper
{
    ...
    private function _renderBundleEntities(Sabai $sabai, $entityType, array $entities, $bundleName, $displayMode)
    {
        ...
        $sabai->doEvent('EntityRenderEntities', array($bundle, $entities, $displayMode));
    ...
    {    
        ...
        as $bundle) {
            // Notify entities are being rendered
            $sabai->doEvent('EntityRenderEntities', array($bundle, $entities_by_bundle[$bundle->name], $displayMode));
    ...
    {        
        ...
        $event_args = array($bundle, $entity, $displayMode, $id, &$classes, &$links);
        $application->doEvent('EntityRenderHtml', $event_args);
        $application->doEvent('EntityRender' . ucfirst($entity_type) . 'Html', $event_args);
        $application->doEvent('EntityRender' . ucfirst($entity_type) . $application->Camelize($entity->getBundleType()) . 'Html', $event_args);
