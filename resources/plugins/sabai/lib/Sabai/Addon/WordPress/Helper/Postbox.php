...
{    
    ...
    private function _renderFormElement($element, $top = true)
    {
        if (!empty($element['elements'])) {
            // Fieldset
            $class = isset($element['class']) ? $element['class'] : '';
            if ($top) {
                $class .= ' sabai-wordpress-postbox-content';
            }
            ...
            foreach ($element['elements'] as $_element) {
                $this->_renderFormElement($_element, false);
            }
            echo '</div>';
            if (isset($element['error'])) {
            ...
            foreach ($element as $_element) {
                echo $this->_doRenderFormElement($_element, $top);
            }
        }
    }

    private function _doRenderFormElement($element, $top = true)
    {
