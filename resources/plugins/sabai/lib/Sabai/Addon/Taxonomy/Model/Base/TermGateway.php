...
abstract class Sabai_Addon_Taxonomy_Model_Base_TermGateway extends SabaiFramework_Model_TreeGateway
{
    ...
    protected function _getInsertQuery(&$values)
    {
        ...
        return sprintf('INSERT INTO %staxonomy_term(term_name, term_title, term_entity_bundle_name, term_entity_bundle_type, term_created, term_updated, term_parent, term_user_id) VALUES(%s, %s, %s, %s, %d, %d, %d, %d)', $this->_db->getResourcePrefix(), $this->_db->escapeString($values['term_name']), $this->_db->escapeString($values['term_title']), $this->_db->escapeString($values['term_entity_bundle_name']), $this->_db->escapeString($values['term_entity_bundle_type']), $values['term_created'], $values['term_updated'], $values['term_parent'], $values['term_user_id']);
    }
    ...
    protected function _beforeDelete1($id, array $old)
    {
        ...
        $this->_db->exec(sprintf('UPDATE %1$staxonomy_term_tree SET tree_path_length = tree_path_length -1 WHERE tree_descendant = (SELECT tree_descendant1 FROM (SELECT tree_descendant AS tree_descendant1 FROM %1$staxonomy_term_tree WHERE tree_ancestor = %d) AS tree_descendant2)', $this->_db->getResourcePrefix(), $old['term_id']));
