        ...
        switch ($this->_name) {
            case 'taxonomy_select':
                ...
                if (empty($bundle->info['taxonomy_hierarchical'])) {
                    ...
                    $options += $this->_addon->getModel('Term')->entityBundleName_is($bundle->name)->fetch(0, 0, 'title', 'ASC')->getArray('title');
                } else {
                    // Hierarchical taxonomy
                    $tree = array('' => $default_text);
                    $options = $this->_addon->getApplication()->Taxonomy_Tree($bundle, '--', $tree);
                }
                return array(
                    '#type' => 'select',
                    '#empty_value' => '',
            ...
            case 'taxonomy_term_parent':
                ...
                return array(
                    ...
                    '#default_value' => $value,
                    '#multiple' => false,
                    '#options' => $this->_addon->getApplication()->Taxonomy_Tree($field->Bundle, '--', array('' => '')),
