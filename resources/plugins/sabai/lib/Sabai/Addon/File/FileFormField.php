<?php
class Sabai_Addon_File_FileFormField implements Sabai_Addon_Form_IField
{
    private $_addon;
    ...
    public function formFieldOnSubmitForm($name, &$value, array &$data, Sabai_Addon_Form_Form $form)
    {
        ...
        // Init uploader
        $uploader = $this->_addon->getUploader(array(
        ...
        if (!empty($data['#multiple'])) {
            ...
            foreach (array_keys($files['name']) as $i) {
                ...
                try {
                    $_file = $uploader->uploadFile($_file);
            ...
            try {
                $file = $uploader->uploadFile($files);
            } catch (Sabai_RuntimeException $e) {
                if ($e->getCode() !== UPLOAD_ERR_NO_FILE) {
                    throw $e;
                }
