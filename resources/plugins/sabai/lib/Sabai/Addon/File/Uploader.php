<?php
class Sabai_Addon_File_Uploader
{
    protected $_addon, $_allowedExtensions = array(),
        $_maxSize, $_maxImageWidth, $_maxImageHeight, $_minImageWidth, $_minImageHeight,
        $_imageExtensions = array('gif', 'jpg', 'jpeg', 'png', 'bmp'), $_imageOnly = false,
        $_uploadDir, $_uploadFileNamePrefix, $_uploadFileNameMaxLength, $_uploadFilePermission = 0644,
        $_hashUploadFileName = true, $_skipMimeTypeCheck = false;
    ...
    public function setAllowedExtensions(array $value)
    {
    ...
    public function setImageExtensions(array $value)
    {
        $this->_imageExtensions = $value;

        return $this;
    }

    public function setImageOnly($value)
    {
    ...
    public function setUploadFileNamePrefix($value)
    {
        $this->_uploadFileNamePrefix = $value;

        return $this;
    }

    public function setUploadFileNameMaxLength($value)
    {
        $this->_uploadFileNameMaxLength = $value;

        return $this;
    }

    public function setUploadFilePermission($value)
    {
        $this->_uploadFilePermission = $value;

        return $this;
    }
    
    public function hashUploadFileName($value = true)
    {
        $this->_hashUploadFileName = (bool)$value;

        return $this;
    }
    
    public function skipMimeTypeCheck($value = true)
    {
        $this->_skipMimeTypeCheck = (bool)$value;

        return $this;
    }
    
    public function uploadFile(array $file, $upload = true)
    {
    ...
    protected function _checkExtensionAndMimeType(&$file)
    {
        ...
        if ('' == $file['file_ext'] = self::_getFileExtension($file['name'])) {
        ...
        if (!$allowed_mime_types = (array)self::_getPossibleMimeTypes($file['file_ext'])) {
        ...
        if (($valid_image_types = (array)@self::$_imageTypes[$file['file_ext']])
            && ($image_type = self::_getImageType($file['tmp_name']))
            && in_array($image_type, $valid_image_types)
        ) {
            $file['is_image'] = true;

            if (!in_array($file['file_ext'], $this->_imageExtensions)) {
    ...
    protected function _checkMaxSize($file)
    {
        if (empty($this->_maxSize)) return;

        if ($file['size'] > $this->_maxSize) {
            throw new Sabai_RuntimeException(sprintf(
                __('File size may not exceed %s KB.', 'sabai'),
                round($this->_maxSize / 1024, 1)
            ));
        }
    }

    protected function _checkMaxImageDimension($file)
    {
        if (!$image_size = @getimagesize($file['tmp_name'])) {
            throw new Sabai_RuntimeException(__('Failed detecting image dimensions.', 'sabai'));
        }

        if ((!empty($this->_maxImageWidth) && $image_size[0] > $this->_maxImageWidth) // check width
            || (!empty($this->_maxImageHeight) && $image_size[1] > $this->_maxImageHeight) // check height
        ) {
            throw new Sabai_RuntimeException(sprintf(
        ...
        if ((!empty($this->_minImageWidth) && $image_size[0] < $this->_minImageWidth) // check width
            || (!empty($this->_minImageHeight) && $image_size[1] < $this->_minImageHeight) // check height
        ) {
            throw new Sabai_RuntimeException(sprintf(
                __('Image size must be larger than W%d x H%d pixels.', 'sabai'),
                $this->_minImageWidth,
                $this->_minImageHeight
            ));
        }
    ...
    protected static function _getFileExtension($fileName)
    {
    ...
    protected static function _getImageType($filePath)
    {
    ...
    protected static function _getUniqueFileName($file, $uploadDir, $fileNamePrefix = '', $fileNameMaxLength = null)
    {
    ...
    protected static function _getPossibleMimeTypes($ext)
    {
        switch ($ext) {
            case 'hqx':
