               ...
               Sabai_Addon_System_IPermissions
{
                ...
                if (!empty($context->taxonomy_bundle->info['taxonomy_hierarchical'])) {
                    ...
                    if ($context->entity->getParentId()) {
                        ...
                        foreach ($parent_terms as $parent_term) {
                            $context->setInfo($parent_term->title, $this->_application->Url($context->taxonomy_bundle->getPath() . '/' . rawurlencode($parent_term->name)));
                        }
                    }
                }
                return true;
        ...
        switch ($path) {
            ...
            case 'edit_term':
                if ((!$id = $context->getRequest()->asInt('entity_id'))
                    || (!$entity = $this->_application->Entity_TypeImpl('taxonomy')->entityTypeGetEntityById($id))
                ) {
                    return false;
                }
        ...
        switch ($path) {
            case 'terms':
                if ($titleType === Sabai::ROUTE_TITLE_TAB_DEFAULT) {
                    return sprintf(__('%s List', 'sabai'), $this->_application->Entity_BundleLabel($context->taxonomy_bundle, true));
                }
                return $this->_application->Entity_BundleLabel($context->taxonomy_bundle, false);
            case 'edit_term':
                if ($titleType === Sabai::ROUTE_TITLE_TAB_DEFAULT) {
        ...
        if ($entityType === 'content') {
            foreach ($bundles as $bundle) {
                ...
                foreach ($bundle->info['taxonomy_terms'] as $taxonomy_bundle_name => $settings) {
                    ...
                    $title = isset($settings['title']) ? $settings['title'] : $this->_application->Entity_BundleLabel($terms_bundle, $taxonomy_max_num_items === 1);
                    ...
                    $this->_application->getAddon('Entity')->createEntityField(
                        ...
                        array(
                            ...
                            'settings' => array(),
                            'title' => isset($settings['content_title']) ? $settings['content_title'] : $this->_application->Entity_BundleLabel($bundle, false),
                            'weight' => 99,
                    ...
                    $this->_application->getAddon('Entity')->createEntityField(
                        ...
                        array(
                            ...
                            'settings' => array(),
                            'title' => isset($settings['content_count_title']) ? $settings['content_count_title'] : sprintf(__('%s count', 'sabai'), $this->_application->Entity_BundleLabel($bundle, true)),
                            'weight' => 99,
                            'data' => array('bundle_id' => $bundle->id, 'bundle_name' => $bundle->name),
                        ),
                        Sabai_Addon_Entity::FIELD_REALM_ALL
            ...
            foreach ($bundles as $bundle) {
                ...
                if (!isset($bundle->info['taxonomy_body']) || false !== $bundle->info['taxonomy_body']) {
                    $this->_application->getAddon('Entity')->createEntityField(
                        ...
                        array(
                            'type' => 'markdown_text',
                            'title' => isset($bundle->info['taxonomy_body']['title']) ? $bundle->info['taxonomy_body']['title'] : __('Body', 'sabai'),
    ...
    public function onEntityCreateEntitySuccess(Sabai_Addon_Entity_Model_Bundle $bundle, Sabai_Addon_Entity_IEntity $entity)
    {
        ...
        // Get terms added
        $terms_updated = array();
        foreach (array_keys($bundle->info['taxonomy_terms']) as $taxonomy_name) {
        ...
        $this->_updateTermsContentCount($terms_updated, $bundle);
    }
    ...
    private function _updateTermsContentCount(array $terms, Sabai_Addon_Entity_Model_Bundle $contentBundle)
    {
        foreach ($terms as $taxonomy_type => $taxonomy_terms) {
            ...
            foreach ($taxonomy_terms as $term) {            
                ...
                $this->_application->getAddon('Entity')->updateEntity($term, array('taxonomy_content_count' => $counts_for_save));
            }
        }
    }
    ...
    public function onEntityRenderTaxonomyHtml($bundle, $entity, $displayMode, $id, &$classes, &$links)
    {
    ...
    private function _onFormBuildContentAdminListposts(&$form)
    {
        ...
        $order = 12;
        foreach ($this->_application->getModel('Bundle', 'Entity')->name_in(array_keys($form['#bundle']->info['taxonomy_terms']))->fetch() as $taxonomy_bundle) {
            ...
            foreach ($form['entities']['#options'] as $entity_id => $data) {
                ...
                foreach ($terms as $key => $term) {
                    $terms[$key] = $this->_application->LinkTo($term->getTitle(), $this->_application->Url($form['#bundle']->getPath(), array('taxonomy_terms['. $taxonomy_bundle->type .']' => $term->getId())));
                }
    ...
    public function onContentAdminPostsQuery($context, $bundle, $query, $count_query, $sort, $order)
    {
        if ($taxonomy_terms = $context->getRequest()->asArray('taxonomy_terms')) {
            foreach ($taxonomy_terms as $taxonomy_term_id) {
                if (!$taxonomy_term = $this->_application->Entity_TypeImpl('taxonomy')->entityTypeGetEntityById($taxonomy_term_id)) {
                    continue;
                }
    ...
    protected function _assignTerm($entities, $taxonomyType, $termId)
    {
        foreach ($this->_application->Entity_Entities('content', $entities) as $entity) {
            if ($entity->getSingleFieldValue($taxonomyType) == $termId) {
                continue; // no change
            }
            $this->_application->getAddon('Entity')->updateEntity($entity, array($taxonomyType => $termId));
        }
    }
    ...
    protected function _removeTerms($entities, $taxonomyType)
    {
        foreach ($this->_application->Entity_Entities('content', $entities) as $entity) {
            ...
            $this->_application->getAddon('Entity')->updateEntity($entity, array($taxonomyType => false));
        }
    }
    ...
    {
        return array('taxonomy_child_terms');
    }

    public function entityGetWidget($name)
    {
        switch ($name) {
            case 'taxonomy_child_terms':
                return new Sabai_Addon_Taxonomy_ChildTermsEntityWidget($this);
        }
    }
