...
{    
    protected function _doGetFormSettings(Sabai_Context $context, array &$formStorage)
    {
        ...
        foreach ($pager->getElements()->with('User') as $comment) {
            ...
            $form['comments']['#options'][$comment->id] = array(
                ...
                'published_at' => $this->DateDiff($comment->published_at),
            ...
            if ($comment->isHidden()) {
                $form['comments']['#row_attributes'][$comment->id]['@row']['class'] = 'sabai-muted';
            }
        }
    ...
    protected function _updateStatus(Sabai_Context $context, $ids, $status)
    {
        ...
        // Notify
        foreach ($comments as $comment) {
            $this->doEvent('CommentDeleteCommentSuccess', array($comment));
        }
    }
