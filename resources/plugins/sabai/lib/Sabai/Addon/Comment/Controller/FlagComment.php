...
class Sabai_Addon_Comment_Controller_FlagComment extends Sabai_Addon_Form_Controller
{
    ...
    target.hide();
    var comment = jQuery("#sabai-comment-%d");
    comment.find(".sabai-comment-vote, .sabai-comment-flag").remove().end()
        .find(".sabai-comment-flags").show().find("span").html("<i class=\"sabai-icon-flag\"></i> " + result.flag.count + " (" + result.flag.sum + ")");
    if (result.hide) {
    ...
    public function submitForm(Sabai_Addon_Form_Form $form, Sabai_Context $context)
    {
        ...
        if (!empty($notify_comment_spam)) {
            $this->doEvent('CommentFlaggedAsSpam', array($context->comment, $context->entity));
        }
    }
}