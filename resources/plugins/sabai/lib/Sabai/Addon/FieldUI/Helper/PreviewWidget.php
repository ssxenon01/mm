        ...
        if ($preview !== ''
            ...
            switch ($max_num) {
                case 1:
                    break;
                case 0:
                    $preview = sprintf('<div>%s<div style="margin-top:10px;"><span class="sabai-btn sabai-btn-mini" ><i class="sabai-icon sabai-icon-plus"></i> %s</span></div></div>', $preview, __('Add More', 'sabai'));
                    break;
                default:
                    $ret = array();
                    for ($i = 0; $i < $max_num; $i++) {
                    ...
                    $preview = '<div>' . implode('</div><div>', $ret) . '</div>';
            }
        }
