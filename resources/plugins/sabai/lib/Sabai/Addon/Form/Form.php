...
class Sabai_Addon_Form_Form
{
    ...
    public function createElement($type, $name, array &$data)
    {
        ...
        if (!isset($data['#label'])) {
            $data['#label'] = array(
                empty($data['#title_no_escape']) ? Sabai::h($data['#title']) : $data['#title'],
        ...
        if (!empty($data['#states'])) {
            ...
            foreach ($data['#states'] as $action => $conditions) {
                $this->settings['#states'][$dependent] = array('action' => $action);
                foreach ($conditions as $dependee => $condition) {
                    // Prepend form ID to the selector if no ID seems to be set
                    if (false === strpos($dependee, '#')) {
                        $dependee = sprintf('#%s %s', $this->settings['#id'], $dependee);
                    }
                    $this->settings['#states'][$dependent]['conditions'][$dependee] = $condition;
                }
            }
        }
    ...
    public function getClickedButtonName()
    {
        ...
        return array_shift(array_keys($this->values[Sabai_Addon_Form::FORM_SUBMIT_BUTTON_NAME]));
    }
    ...
    public function submit($values = array())
    {
        $this->values = $values;
    ...
    private function _doSubmit()
    {
        ...
        if (!empty($this->settings['#force_submit'])) {
        ...
        foreach (array_keys($this->_elements['#children']) as $weight) {
            foreach (array_keys($this->_elements['#children'][$weight]) as $ele_name) {
                ...
                try {
                    ...
                    $this->_addon->getApplication()->Form_FieldImpl($ele_type)
    ...
    private function _doRender()
    {
        ...
        // Add form footer if any
        if (!empty($this->settings['#footer'])) {
            foreach ((array)$this->settings['#footer'] as $footer) {
                $this->_htmlquickform->addFooter($footer);
            }
        }
        // Add form JS if any
        ...
        if (!empty($this->settings['#states'])) {
            $this->addJs(sprintf(
                'jQuery(document).ready(function(){SABAI.states(%s);});',
                json_encode($this->settings['#states'])
            ));
        }

        $this->_renderer->setClassPrefix('sabai-');
            ...
            foreach ($this->_errors as $element_name => $messages) {
                if (strlen($element_name) === 0) {
                    ...
                    foreach ($messages as $message) {
                        $this->_htmlquickform->addHeader(sprintf('<div class="sabai-error">%s</div>', Sabai::h($message)));
                    }
                    $header_added = true;
                } else {
            ...
            foreach ($_elements as $ele_name => $ele_data) {
                try {
                    $this->_addon->getApplication()->Form_FieldImpl($ele_data['#type'])
                        ->formFieldOnRenderForm($ele_name, $ele_data, $this);
                } catch (Sabai_IException $e) {
                    $this->setError($e->getMessage());
                } catch (Exception $e) {
                    // Do not display system error messages to the user
    ...
    public function renderElement($data)
    {
        ...
        // Required?
        if ($data['#required'] || !empty($data['#display_required'])) {
        ...
        if (isset($data['#template'])) {
            if (false === $data['#template']) {
                $this->_renderer->setElementTemplate('{element}', $data['#name']);
            } else {
        ...
        if (!empty($data['#js'])) {
            // Generate an ID for this element is not already set
            if (!isset($data['#id'])) {
                $data['#id'] = $this->getFieldId($data['#name']);
            }
            foreach ((array)$data['#js'] as $js) {
                $this->addJs(sprintf($js, $data['#id']));
            }
        }
    }
