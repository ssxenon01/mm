...
abstract class Sabai_Addon_System_Controller_Admin_AccessControl extends Sabai_Addon_Form_Controller
{
    ...
    protected function _getPermissionFormSettings(Sabai_Context $context, array $categories)
    {
        ...
        foreach ($this->getModel('PermissionCategory', 'System')->name_in($categories)->fetch()->with('Permissions') as $category) {
            // Create grid table
            $form['normal'][$category->name] = array(
                '#type' => 'grid',
                '#collapsible' => false,
                ...
                '#weight' => array_search($category->name, $categories),
                'label' => array(
                    '#type' => 'item',
            ...
            foreach ($this->_roles as $role_name => $role) {
                ...
                $form['normal'][$category->name][$role->name] = array(
                    '#type' => 'checkbox',
                    '#title' => $role_title,
                    '#disabled' => $is_admin_role,
                    '#weight' => $is_admin_role ? 0 : ++$role_weight,
                );
                $form['normal'][$category->name]['#column_attributes'][$role->name]
                    = array('style' => 'width:' . round(60 / count($this->_roles)) .'%');
            }
            // Add rows
            foreach ($category->Permissions as $permission) {
                $form['normal'][$category->name]['#default_value'][$permission->name]
                    = array('label' => $permission->title,);
                foreach ($this->_roles as $role_name => $role) {
                    if (isset($this->_adminRoles[$role_name])) {
                        $form['normal'][$category->name]['#default_value'][$permission->name][$role_name] = 1; 
                    } elseif ($role->isGuest()) {
                        if (!$permission->guest_allowed) {
                            $form['normal'][$category->name]['#row_settings'][$permission->name][$role_name]
                                = array('#attributes' => array('disabled' => 'disabled'));
                        } else {
                            $form['normal'][$category->name]['#default_value'][$permission->name][$role_name]
                                = !empty($role_permissions[$role_name][$permission->name]);
                        }
                    } else {
                        $form['normal'][$category->name]['#default_value'][$permission->name][$role_name] = !empty($role_permissions[$role_name][$permission->name]);
                    }
                }
