...
class Sabai_Addon_System_Controller_Admin_ReloadAddon extends Sabai_Addon_Form_Controller
{
    ...
    protected function _doGetFormSettings(Sabai_Context $context, array &$formStorage)
    {
        ...
        $this->_submitButtons[] = array('#value' => __('Upgrade Add-on', 'sabai'), '#btn_type' => 'info');
        $message = sprintf(__('Press the button below to upgrade <strong>%s</strong> to version <strong>%s</strong>.', 'sabai'), Sabai::h($addon_name), Sabai::h($this->_addon->version));
        $form = array(
        ...
        $form['config'] += $this->getAddon($this->_addon->name)->getUpgradeForm($this->_addon->version, array('config'));
    ...
    public function submitForm(Sabai_Addon_Form_Form $form, Sabai_Context $context)
    {
        $log = new ArrayObject();
        $current_version = $this->_addon->version;
        $this->doEvent('SabaiAddonReload', array($this->_addon, $log));
        $this->UpgradeAddon($this->_addon, array(), $log);
        $this->reloadAddons()->doEvent('SabaiAddonUpgraded', array($this->_addon, $current_version, $log));
        $this->getPlatform()->clearCache();
        foreach ($log as $_log) {
            $context->addFlash($_log);
        }
        $context->setSuccess($this->Url('/settings', array('refresh' => 0, 'enable_reload' => 1)))
