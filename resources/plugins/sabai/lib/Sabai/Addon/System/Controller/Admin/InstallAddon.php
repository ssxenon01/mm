...
class Sabai_Addon_System_Controller_Admin_InstallAddon extends Sabai_Addon_Form_Controller
{
    ...
    protected function _doGetFormSettings(Sabai_Context $context, array &$formStorage)
    {
        ...
        $form['config'] += $this->fetchAddon($this->_addonName)->getInstallForm(array('config'));
    ...
    {
        $log = new ArrayObject();
        $addon = $this->InstallAddon($this->_addonName, (array)@$form->values['config']);
        $this->reloadAddons() // Refresh addons to include the installed addon during the installed event
            ->doEvent('SabaiAddonInstalled', array($addon, $log));
