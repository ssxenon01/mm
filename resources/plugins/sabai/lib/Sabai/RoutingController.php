...
abstract class Sabai_RoutingController extends SabaiFramework_Application_RoutingController
{
    ...
    protected function __construct(Sabai_Route $defaultRoute)
    {
        parent::__construct($defaultRoute, true);
    }
    ...
    protected function _isRoutable(SabaiFramework_Application_Context $context, $route)
    {
        ...
        if ($dot_pos = strpos($route, '.')) {
            ...
            if (in_array($content_type, array('html', 'xml', 'json'))) {
        ...
        $this->setCurrentAddon($route_matched['addon']);
        ...
        while (($routes = @$route_matched['routes'])
            ...
            //if (!empty($tabs)) {
                $tabs[0][''] = array(
                    ...
                    'clear_tabs' => !empty($route_matched['data']['clear_tabs'])
                );
            //}
            ...
            if (isset($routes[$path_requested])) {
                ...
                if (!$matched) {
                    if ($this->_addTabs($context, $tabs)) $context->setCurrentTab('');
                    $this->_addMenus($context, $menus, $paths_matched);
                    $this->_setInlineTabs($context, $inline_tabs, $paths_matched);
                    break;
                }
            ...
            if ($route_matched['type'] != Sabai::ROUTE_CALLBACK) {
                ...
                foreach (array_reverse($breadcrumbs) as $bc) {
                    $context->setInfo($bc['title'], $this->Url($bc['url']), $bc['ajax']);
                }
            }
        ...
        if (empty($last_valid_route_matched)) {
            $context->setNotFoundError();
        ...
        if ($last_valid_route_matched['addon'] !== (string)$this->getAddon()) {
            $this->setCurrentAddon($this->getAddon($last_valid_route_matched['addon']));
        }

        if (!empty($last_valid_route_matched['method'])
            && strcasecmp(Sabai_Request::method(), $last_valid_route_matched['method']) !== 0
        ) {
        ...
        if (!empty($last_valid_route_matched['forward'])) {
            ...
            if ($last_valid_route_matched['type'] == Sabai::ROUTE_INLINE_TAB
                ...
                $params = array(Sabai_Request::$inlineTabParam => basename($route_matched)) + $context->getRequest()->getParams();
                ...
                $fragment = empty($params['__fragment']) ? 'sabai-inline-nav' : $params['__fragment'];
            ...
            if (empty($last_valid_route_matched['controller'])) {
                $context->setRedirect($this->_application->getPlatform()->getHomeUrl());
                
                return false;
            }

            $this->_processRouteData($last_valid_route_matched);
        }

        return new Sabai_Route('/' . implode('/', $paths_matched) . '/', $last_valid_route_matched);
    }
    ...
    private function _addTabs(Sabai_Context $context, array $tabs)
    {
        ...
        if ($this->_tabsAdded && !empty($tabs[0]['']['clear_tabs'])) {
            $context->clearTabs();
        }
        ...
        foreach ($inlineTabs as $route_key => $route_data) {
            ...
            if ($title = $this->_getTitle($context, $route_data, Sabai::ROUTE_TITLE_TAB)) {
                $inline_tabs[$route_data['weight']][$route_key] = array(
                    'ajax' => $route_data['ajax'],
                    'title' => $title,
                    'url' => array('route' => '/' . implode('/', $pathsMatched) . '/', 'params' => array(Sabai_Request::$inlineTabParam => $route_key), 'fragment' => 'sabai-inline-nav'),
                    'class' => $route_data['class'],
                    'addon' => $route_data['addon'],
                    'route' => new Sabai_Route('/' . implode('/', $pathsMatched) . '/' . $route_key . '/', $route_data),
                    'featured' => !empty($route_data['data']['featured']),
                    'disabled' => !empty($route_data['data']['disabled']),
                    'options' => isset($route_data['data']['link_options']) ? $route_data['data']['link_options'] : array(),
                );
