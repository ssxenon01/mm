...
class Sabai_Addon_Directory_Controller_Feed extends Sabai_Addon_Content_Controller_Feed
{
    ...
    protected function _getDescription(Sabai_Context $context)
    {
        ...
        return sprintf($title, $this->_numItems, $this->getAddon()->getDirectoryPageTitle());
    }
    ...
    protected function _getItemTitle(Sabai_Context $context, Sabai_Addon_Content_Entity $entity)
    {
        switch ($entity->getBundleType()) {
            case 'directory_listing_review':
                $listing = $this->Content_ParentPost($entity);
                return $listing ? sprintf('%s - Review of %s (%.1f/5)', $entity->getTitle(), $listing->getTitle(), $entity->directory_rating[0]) : '';
            default:
                return $entity->getTitle();
        }
    }
    
    protected function _getItemExtras(Sabai_Context $context, Sabai_Addon_Content_Entity $entity)
    {
        switch ($entity->getBundleType()) {
            case 'directory_listing':
                return array('georss:point' => $entity->getSingleFieldValue('directory_location', 'lat') . ' ' . $entity->getSingleFieldValue('directory_location', 'lng'));
            default:
