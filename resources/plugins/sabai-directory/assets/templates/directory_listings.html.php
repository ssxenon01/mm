...
jQuery(document).ready(function($) {
    jQuery(".sabai-directory-search-submit").click(function(e){
        ...
        SABAI.ajax({
            ...
            data: "<?php unset($url_params['center'], $url_params['is_geolocate']); echo http_build_query($url_params);?>&" + form.serialize()
        });
        e.preventDefault();
    }); 
});
...
<?php else:?>
    <?php $this->renderTemplate('directory_listings_' . $settings['view'], array('entities' => $entities, 'paginator' => $paginator, 'url_params' => $url_params, 'sorts' => $sorts, 'views' => $views, 'distances' => $distances, 'center' => $center, 'settings' => $settings, 'is_drag' => $is_drag, 'is_geolocate' => $is_geolocate, 'geocode_error' => $geocode_error));?>
<?php endif;?>
</div>
